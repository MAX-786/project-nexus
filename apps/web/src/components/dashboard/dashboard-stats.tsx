...
export default function DashboardStats({ dueCount }: DashboardStatsProps) {
  ...
  return (
    <div className="border-b border-border/50 bg-background/60 backdrop-blur-sm" role="region" aria-label="Dashboard statistics">
      <div className={cn('grid gap-3 px-6 py-3 stagger-children', stats.length > 4 ? 'grid-cols-2 lg:grid-cols-5' : 'grid-cols-2 lg:grid-cols-4')} role="list" aria-label="Key metrics">
        {stats.map((stat) => (
